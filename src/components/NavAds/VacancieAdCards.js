...
function VacancieAdCards() {
    ...
    useEffect(() => {
        const fetchAds = async () => {
            try {
                const adsCollectionRef = collection(db, 'categories', 'Vacancies', 'ads');
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-[90%] m-auto">
            {loading ? (
                ...
                ads.map(ad => (
                    <LazyLoad key={ad.id} height={380} offset={100}>
                        <Link to={`/ad-details/${ad.id}`} key={ad.id}>
                            <div className="border-2 rounded-md overflow-hidden h-[380px] w-full sm:w-[260px] xl:w-80">
                                {ad.promoted && (
                                ...
                                <div className="p-3">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <p className="text-xs text-gray-500 items-end mt-4 self-end">
                                            {ad && ad.timestamp && new Date(ad.timestamp.seconds * 1000).toLocaleString('en-US', {  day: 'numeric', year: 'numeric', month: 'long' }).toUpperCase()}
                                        </p>
                                    </div>
                                </div>
                            </div>
...
export default VacancieAdCards;
